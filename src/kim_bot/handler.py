...
from kim_bot.rag import is_rag_required, create_rag_query, extra_knowledge
...
class Conversation:
    def __init__(self):
        ...
        self.add_history(str(OllamaRole.system), content=self.system_prompt)
    ...
    def chat_response(
        self, stream: bool = False, format: str = None, save: bool = True
    ) -> None:
        ...
        if save:
            self.add_history(role=str(OllamaRole.assistant), content=response)

    def generate_response(self, role: OllamaRole, user_input: str) -> None:
        query = user_input

        if is_rag_required(query):
            query = create_rag_query(
                history=self.conversation_history, query=user_input
            )

            query = extra_knowledge(query)
